...
p5.prototype.measureText = function (text) {
  const ctx = this._renderer.drawingContext;
  const bounds = ctx.measureText(text);
  const vertical = {
    ascent: bounds.actualBoundingBoxAscent,
  ...
  const _width = bounds.actualBoundingBoxLeft + bounds.actualBoundingBoxRight;
  const verticalOffset = (vertical.ascent - vertical.descent) * 0.5;
  const horizontalOffset = (horizontal.left - horizontal.right) * 12;
...
p5.prototype.textVariations = function (theVariations) {
  if (theVariations) {
    if (typeof theVariations === "number") {
      ...
      let formatedfontVariationsSettings = fontVariationsSettings.slice(0, -1);
...
p5.prototype.textFont = function (theFont, theSize, theVariations) {
  ...
  if (arguments.length) {
    ...
    if (theVariations) {
      ...
      this._renderer._setProperty("_fontVariations", "");
      this._renderer._setProperty("_textWeight", "");
    }
