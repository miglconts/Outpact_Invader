...
function preload() {
  outpactFont = loadFont("Outpact-VF.ttf");
}

// --- SETUP ---
function setup() {
  createCanvas(windowWidth, windowHeight);
  pixelDensity(1); // opcional: evita que canvas se duplique en retina
  registerFont();
  player = new Player();
...
function drawStartScreen() {
  ...
  textSize(infoSize);
  textVariations({ wght: 400 });
  text(
    "Arrastra el dedo o mueve el mouse para moverte.",
    width / 2,
    height * 0.5 + infoSize
  );
  text(
    "Toca o haz clic para disparar.",
    width / 2,
    height * 0.5 + (infoSize * 2 + 3)
  );
...
function drawButton(label) {
  ...
  fill(0);
  let txtSize = constrain(width * 0.04, 18, 28);
  textSize(txtSize);
  textVariations({ wght: 700 });
  textAlign(CENTER, CENTER);
  text(label, width / 2, buttonY + buttonH / 2);
}

// --- CONTROLES ---
function isOverButton(x, y) {
  return (
    x > buttonX && x < buttonX + buttonW && y > buttonY && y < buttonY + buttonH
  );
}
...
function touchEnded() {
  if (gameState === "playing" && !gameJustStarted)
...
class Bullet {
  constructor(x, y) {
    this.x = x;
    this.y = y;
    this.r = constrain(width * 0.007, 5, 10);
